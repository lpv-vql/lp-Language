...
それぞれの値は省略することができます。リストや文字列の操作などの場合は終値を省略するとリストや文字列の長さになります。
```
print 0 ~ 10 .. 2;
...
## 文字列の操作
文字列も[リスト](#リスト)と同じような操作をすることができます。

変数`text`には`"ABCDEFG"`が入っています。
```
text = "ABCDEFG"
```
...
>>> ABCDEFg
```
左側のものを右側のものに置換します。同じ文字が複数ある場合は、すべて置換されます。
...
>>> ABCDEF
```
このように記述することで、文字列に含まれる特定の文字を削除することができます。

複数同じ文字がある場合もすべて削除されます。
...
>>> 2
```
[要素の取得](#要素の取得)の時に`{ }`の中の最初に`#`を入れると文字の場所を取得することができます。

同じ文字が複数ある場合は返り値が[リスト](#リスト)になります。
...
>>> true
```
`in`で、文字が文字列に含まれているかを調べることができます。
...
>>> 1
```
文字列の中に文字が何個含まれるかを調べます。

### 文字列の演算
`+` `-` `*` の[演算子](#演算)は文字列の場合も使うことができます。
```
print "ABC" + "DEF";
